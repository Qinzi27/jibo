...
(function () {
  ...
  function footer() { return '<footer class="footer"><span>LOCAL FIRST. ZERO ACCOUNT.</span><span>肌薄 · 把训练练成日常 · v1.3.0</span></footer>'; }
  ...
  function header() {
    return `<header class="topbar"><div class="brand"><div class="brand-icon">${icon('dumbbell')}</div><div><strong>肌薄</strong><div class="eyebrow">JIBO · FIND YOUR FORM</div></div></div>${nav()}<div class="top-actions"><span class="offline">本地离线</span><button type="button" class="icon-btn" data-action="settings" aria-label="设置与数据备份">${icon('settings')}</button></div></header>`;
  }
  ...
  let theoryAuthor = 'all';
  const theoryAsides = ['理论已看懂，今天先把一组练明白。','先收藏，再开练。第二步不要省略。','嘴上练的是版本，手上练的是哑铃。','薄肌可以慢慢来，动作先别糊弄。'];
  let theoryAside = 0;
  function theoryImage(post) { return window.LEAN_EMBEDDED_IMAGES?.[post.image] || post.image; }
  function theoryPage() {
    const all=window.JIBO_THEORY_POSTS||[], posts=all.filter(p=>theoryAuthor==='all'||p.authorId===theoryAuthor);
    return `${pageHead('THE VERY UNOFFICIAL READING ROOM','肌薄理论','先读一点抽象，再练一点具体。')}<section class="theory-hero"><div><span class="theory-sticker">本馆不授予学位</span><h2>脑子：会了。<br>身体：等等。</h2><p>孙宇晨 × 邵艾伦 · 薄肌话题原帖收藏</p></div><div class="theory-seal" aria-hidden="true">肌薄<br><b>研习室</b><small>EST. 2026</small></div></section><div class="theory-aside"><span>${icon('bolt')}肌薄旁白</span><p id="theory-aside-text">${esc(theoryAsides[theoryAside])}</p><button class="icon-btn" data-action="theory-shuffle" aria-label="换一句肌薄旁白">${icon('arrow')}</button></div><div class="theory-toolbar"><div class="filter-row" aria-label="按原帖作者筛选">${[['all','全部原帖'],['sun','孙宇晨'],['alan','邵艾伦']].map(([id,name])=>`<button class="filter ${theoryAuthor===id?'active':''}" data-action="theory-filter" data-author="${id}" aria-pressed="${theoryAuthor===id}">${name}</button>`).join('')}</div><span class="tiny-note">${posts.length} 张馆藏 · 点击看大图</span></div><div class="theory-grid">${posts.map((post,i)=>`<article class="theory-card" data-post-id="${esc(post.id)}"><div class="theory-card-meta"><span class="theory-author-icon ${post.authorId==='sun'?'sun':'alan'}" aria-hidden="true">${post.authorId==='sun'?'孙':'邵'}</span><div><strong>${esc(post.author)}</strong><small>@${esc(post.handle)}</small></div><span class="theory-issue">NO. ${String(all.indexOf(post)+1).padStart(2,'0')}</span></div><button class="theory-picture" data-action="theory-image" data-id="${esc(post.id)}" aria-label="放大${esc(post.author)}的${esc(post.title)}原帖截图"><img src="${esc(theoryImage(post))}" alt="${esc(post.author)}的 X 原帖截图：${esc(post.excerpt)}" loading="lazy"><span>${icon('search')}看原帖截图</span></button><div class="theory-card-body"><div class="theory-post-date">${esc(post.date)} · ${post.cropped?'原帖节选截图':'原帖截图'}</div><h3>${esc(post.title)}</h3><p><span>肌薄旁白</span>${esc(post.commentary)}</p><button class="btn btn-ghost btn-small" data-action="theory-source" data-id="${esc(post.id)}">${icon('copy')}复制原帖链接</button></div></article>`).join('')}</div><p class="theory-footnote">截图保留原作者与出处，离线可看。原帖是网络表达，不是训练建议；“肌薄旁白”为本应用自制，与原作者无关。</p><button class="btn btn-secondary btn-wide" data-nav="plans">理论学完了，去看训练计划 ${icon('arrow')}</button>`;
  }
  function theoryDetail(id) {
    const post=(window.JIBO_THEORY_POSTS||[]).find(p=>p.id===id); if(!post)return;
    modal(`<div class="eyebrow">X / ${post.cropped?'原帖节选':'原帖截图'}</div><h2>${esc(post.title)}</h2><p class="tiny-note">${esc(post.author)} · @${esc(post.handle)} · ${esc(post.date)}</p>`,`<div class="theory-lightbox"><img src="${esc(theoryImage(post))}" alt="${esc(post.author)}原帖：${esc(post.excerpt)}"></div><p class="theory-source-url">${esc(post.sourceUrl)}</p><div class="modal-actions"><button class="btn btn-secondary" data-action="theory-zoom" aria-pressed="false">放大阅读</button><button class="btn btn-primary" data-action="theory-source" data-id="${esc(post.id)}">${icon('copy')}复制原帖链接</button></div>`);
  }
  ...
  function handleAction(button) {
    ...
    else if(a==='theory-filter'){theoryAuthor=['sun','alan'].includes(button.dataset.author)?button.dataset.author:'all';const y=window.scrollY;render();window.scrollTo(0,y);}
