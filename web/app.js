...
(function () {
  ...
  function rememberScroll() { if(activeWorkout()){runtime.workoutScroll=window.scrollY;saveRuntime();}else navScroll[tab]=window.scrollY; }
  ...
  function footer() { return '<footer class="footer"><span>LOCAL FIRST. ZERO ACCOUNT.</span><span>肌薄 · 把训练练成日常 · v1.2.1</span></footer>'; }
  function nav(mobile = false) {
    return `<nav class="${mobile ? 'mobile-nav' : 'desktop-nav'}" aria-label="${mobile ? '底部导航' : '主导航'}">${[['home','dumbbell','训练'],['plans','calendar','计划'],['library','book','动作'],['progress','chart','进度'],['food','food','饮食']].map(([id,ic,name]) => `<button type="button" class="nav-item ${tab===id?'active':''}" data-nav="${id}" aria-current="${tab===id?'page':'false'}">${icon(ic)}<span>${name}</span></button>`).join('')}</nav>`;
  }
  function header() {
  ...
  function render() {
    ...
    const pages = {home:homePage, plans:plansPage, library:libraryPage, food:foodPage, progress:progressPage, settings:settingsPage};
    ...
    window.scrollTo({top:activeWorkout()?runtime.workoutScroll:(navScroll[tab]||0),behavior:'instant'});
    syncBackState();syncKeyboard();
  }
  function go(next) {rememberScroll();closeModal();tab=next;if(next==='home')overview=false;render();}
  ...
  function plansPage() {
    ...
    return `${pageHead('A ROUTINE YOU CAN KEEP','你的薄肌训练计划。','使用 A / B / C 起步，也可以用喜欢的动作搭配自己的计划。')}
  ...
  async function copyText(text) {
    if(isNative()){window.LeanNative.copyText(text);toast('战报已复制。');return;}
    try{await navigator.clipboard.writeText(text);toast('战报已复制。');}
    catch(_){const t=document.createElement('textarea');t.value=text;t.style.position='fixed';t.style.opacity='0';document.body.appendChild(t);t.select();const ok=document.execCommand('copy');t.remove();if(ok)toast('战报已复制。');else{modal('<h2>长按或选择以下文字复制</h2>',`<textarea readonly style="min-height:260px">${esc(text)}</textarea>`);}}
  }
