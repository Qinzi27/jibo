...
(function () {
  ...
  const APP_VERSION = '1.5.2', RELEASES_URL = 'https://github.com/Qinzi27/jibo/releases';
  ...
  const featuredQuoteIds = ['alan-victory','alan-blond-thinking','alan-room-meme','alan-thin-club'];
  ...
  function requestFinish() {
    ...
    modal('<h2>保存本次训练？</h2>', `<div class="finish-summary"><strong>${stats.sets}<small> 组已完成</small></strong><p>${stats.exercises} 个已练动作 · ${stats.reps} 次${stats.seconds?' · '+stats.seconds+' 秒':''}</p></div><p class="notice">${total>stats.sets?`还有 ${total-stats.sets} 组未标记完成。只统计已完成的组，未完成内容保留为记录，不计入训练量。`:'保存后计入训练记录。'}</p><div class="modal-actions"><button class="btn btn-ghost" data-action="close-modal">继续训练</button><button class="btn btn-primary" data-action="confirm">保存并结束</button></div>`);
  ...
  function footer() { return `<footer class="footer"><span>肌薄</span><span>v${APP_VERSION}</span></footer>`; }
  ...
  function header() {
    return `<header class="topbar"><div class="brand"><div class="brand-icon"><img src="${esc(localImage('assets/icon-512.png'))}" alt="" width="44" height="44"></div><div><strong>肌薄</strong><div class="eyebrow">JIBO</div></div></div>${nav()}<div class="top-actions"><span class="offline">数据在本机</span><button type="button" class="icon-btn settings-button" data-action="settings" aria-label="设置与数据备份${hasAvailableUpdate()?'，有软件更新':''}">${icon('settings')}<span class="update-badge" ${hasAvailableUpdate()?'':'hidden'} aria-hidden="true"></span></button></div></header>`;
  }
  function pageHead(title,desc = '') {
    return `<div class="page-head"><div><h1>${title}</h1>${desc?`<p class="desc">${desc}</p>`:''}</div><div class="date-chip">${icon('calendar')}<span>${today().replaceAll('-','.')}</span></div></div>`;
  }
  function render() {
  ...
  function weekCard() {
    ...
    return `<section class="panel week-panel"><div class="panel-title"><h3>本周训练</h3><span class="chip">参考：3 天</span></div><div class="week-value"><strong>${week.days}</strong><span>/ 3 <small>天已练</small></span></div><div class="week-days" aria-label="本周已训练 ${week.days} 天">${dates.map((date,i)=>`<div class="week-day ${week.dates.includes(date)?'complete':''} ${date===today()?'today':''}"><span>${['一','二','三','四','五','六','日'][i]}</span><b>${week.dates.includes(date)?icon('check'):date.slice(-2)}</b></div>`).join('')}</div><p class="footnote">${week.days>=3?'本周已训练至少 3 天。':'A / B / C 隔日轮换，休息日自由安排。'}<br>仅计入已归档且有完成组的日期。</p></section>`;
  }
  function homePage() {
    ...
    const post=(window.JIBO_THEORY_POSTS||[]).find(p=>p.id===featuredQuoteIds[new Date().getDate()%featuredQuoteIds.length]);
    return `${pageHead('训练')}
      <div class="dashboard"><div class="main-col">
        <section class="hero jibo-hero quote-hero"><div class="hero-copy">${state.profile.fun&&post?`<div class="eyebrow">原帖引用</div><blockquote id="quote" cite="${esc(post.sourceUrl)}">${esc(post.excerpt)}</blockquote><button class="quote-attribution" data-action="theory-image" data-id="${esc(post.id)}">${esc(post.author)} · @${esc(post.handle)} ${icon('arrow')}</button>`:'<h2>今日训练</h2>'}<button class="btn hero-cta" data-action="${hasDraft?'continue-workout':'plan-detail'}" ${hasDraft?'':`data-plan="${recommended.id}"`}>${hasDraft?'继续本次训练':'选择今日计划'} ${icon('arrow')}</button></div></section>
        <section class="next-workout"><div class="plan-letter">${recommended.letter}</div><div><div class="eyebrow">下一次训练</div><h3>${esc(recommended.name)}</h3><p>${recommended.location==='gym'?'健身房':'居家'} · ${recommended.blocks.length} 个动作 · 约 ${recommended.durationMinutes} 分钟</p></div><button class="quiet-icon" data-nav="plans" aria-label="选择训练计划">${icon('arrow')}</button></section>
        <div class="section-head" id="workout-heading"><div><h2>本次训练 <span class="pill-label" id="draft-count">${state.draft ? C.sessionStats(state.draft, EX).sets : 0} 组完成</span></h2></div><div class="row"><button class="btn btn-ghost btn-small" data-action="timer">${icon('clock')}计时</button><button class="btn btn-primary btn-small" data-nav="library">${icon('plus')}加动作</button></div></div>
        <div id="draft-area">${hasDraft?`<div class="empty"><h3>未结束的训练</h3><p>已完成 ${C.sessionStats(state.draft,EX).sets} 组。</p><button class="btn btn-primary" data-action="continue-workout">继续本次训练 ${icon('arrow')}</button></div>`:draftHTML()}</div>
        <div class="explainer">${icon('lock')}<p>填写每组数据，勾选完成。记录自动保存在本机。</p></div>
      </div><aside class="aside-col">
        ${weekCard()}
        <section class="panel aside-card training-basics"><h3>使用说明</h3><p>选计划 → 记录每组 → 保存训练。</p><button class="btn btn-ghost btn-wide" data-action="training-guide">训练说明 ${icon('arrow')}</button><button class="btn btn-ghost btn-wide" data-nav="theory">肌薄理论 ${icon('arrow')}</button></section>
        <section class="panel aside-card"><div class="panel-title"><h3>今日记录</h3></div><div class="mini-stats today-stats"><div><strong>${dayStats.sets}<small> 组</small></strong><span class="tiny-note">已完成并归档</span></div><div><strong>${fmt(dayStats.volume,0)}</strong><span class="tiny-note">负重 × 次数</span></div></div></section>
      </aside></div>`;
  }
  ...
  function plansPage() {
    return `${pageHead('计划与进度')}<section class="plans-hub"><div class="plan-view-switch" role="group" aria-label="计划与进度">${[['plans','calendar','训练计划'],['progress','chart','我的进度']].map(([id,ic,label])=>`<button type="button" class="${planView===id?'active':''}" data-action="plan-view" data-view="${id}" aria-pressed="${planView===id}" aria-controls="plan-view-content">${icon(ic)}${label}</button>`).join('')}</div><div id="plan-view-content" class="plan-view-content">${planView==='progress'?progressPage():planCatalogPage()}</div></section>`;
  }
  function planCatalogPage() {
    ...
    return `<p class="plan-view-intro">选择内置计划，或新建计划。</p>
  ...
  function trainingGuide() {
    modal('<h2>训练说明</h2>', `<div class="stack small muted"><p>内置 A / B / C 计划可隔日轮换，动作、组数和休息时间均可修改。</p><p>先热身，选择能控制的重量；疼痛时停止动作。</p><p>填写实际重量、次数或秒数，再勾选完成。保存训练后可在「计划 → 我的进度」查看。</p></div>`);
  }
  function draftHTML() {
    if (!state.draft || !state.draft.blocks.length) return `<div class="empty"><div class="empty-icon">${icon('dumbbell')}</div><h3>还没有开始训练</h3><p>选择计划，或添加动作。</p><div class="row wrap"><button class="btn btn-primary" data-nav="plans">选择训练计划 ${icon('arrow')}</button><button class="btn btn-ghost" data-nav="library">自由添加动作</button></div></div>`;
    const draft = state.draft;
    const editing = state.sessions.some(s => s.id === draft.id);
    return `<div class="draft-top"><label class="field">训练日期<input type="date" id="draft-date" min="2000-01-01" max="2100-12-31" value="${esc(draft.date)}" required></label><span class="tiny-note">${editing?'正在编辑旧记录；归档会更新原记录。':'草稿自动保存'}</span></div><div class="stack">${draft.blocks.map(blockHTML).join('')}</div><label class="field" style="margin-top:16px">本次备注<textarea id="draft-note" maxlength="${1000-notePrefix(draft).length}" placeholder="器械设置、感受、下次提醒……">${esc(visibleNote(draft))}</textarea></label><div class="save-bar"><button class="btn btn-primary" data-action="finish">${icon('check')}${editing?'保存修改并归档':'结束训练并归档'}</button><button class="btn btn-ghost" data-action="discard-draft">放弃草稿</button></div>`;
  }
  function blockHTML(block) {
    ...
    const details = prev ? `${prev.date} · ${prev.block.sets.filter(s=>s.done).map(s=>ex.mode==='time'?s.seconds+'秒':(ex.mode==='weight'?s.weight+'kg × ':'')+s.reps+'次').slice(0,5).join(' / ')}` : '暂无历史记录';
  ...
  async function loadExercisePhoto(file) {
    ...
    try {
      ...
      $('#photo-status').textContent='照片已选好，请保存动作。';
  ...
  function saveCustomExercise() {
    ...
    closeModal();group='自建';search='';go('library');toast('动作已保存在本机。');
  }
  ...
  function saveCustomPlan() {
    ...
    closeModal();planLocation='custom';go('plans');toast('计划已保存。');
  }
  ...
  function filteredTheoryPosts() {
    ...
    return (window.JIBO_THEORY_POSTS||[]).filter(post=>(theoryAuthor==='all'||post.authorId===theoryAuthor)&&(theoryCategory==='all'||(post.category||'theory')===theoryCategory)&&(!query||[post.author,post.handle,post.title,post.excerpt,theoryCategoryLabel(post)].join(' ').toLocaleLowerCase().includes(query)));
  }
  function theoryResults() {
    const all=window.JIBO_THEORY_POSTS||[], posts=filteredTheoryPosts();
    if(!posts.length)return '<div class="empty theory-empty"><div class="empty-icon">'+icon('search')+'</div><h3>没有找到原帖</h3><p>修改关键词或清除筛选。</p><button class="btn btn-secondary" data-action="theory-clear">查看全部原帖</button></div>';
    return `<div class="theory-grid">${posts.map(post=>{const images=theoryImages(post);return `<article class="theory-card" data-post-id="${esc(post.id)}"><div class="theory-card-meta"><span class="theory-author-icon ${post.authorId==='sun'?'sun':'alan'}" aria-hidden="true">${post.authorId==='sun'?'孙':'邵'}</span><div><strong>${esc(post.author)}</strong><small>@${esc(post.handle)}</small></div><span class="theory-issue">${String(all.indexOf(post)+1).padStart(2,'0')}</span></div><button class="theory-picture" data-action="theory-image" data-id="${esc(post.id)}" aria-label="放大${esc(post.author)}的${esc(post.title)}${post.cropped?'原帖节选':'原帖截图'}${images.length>1?'，共'+images.length+'张':''}"><img src="${esc(theoryImage(post))}" alt="${esc(post.author)}的 X ${post.cropped?'原帖节选':'原帖截图'}：${esc(post.excerpt)}" loading="lazy"><span>${icon('search')}${images.length>1?images.length+' 张截图':'查看截图'}</span></button><div class="theory-card-body"><div class="theory-post-date">${esc(post.date)} · ${esc(theoryCategoryLabel(post))} · ${post.cropped?'原帖节选':'原帖截图'}</div><blockquote cite="${esc(post.sourceUrl)}">${esc(post.excerpt)}</blockquote>${post.id==='alan-thin-club'?'<small class="media-note">原帖标注：Made with AI</small>':''}<button class="btn btn-ghost btn-small" data-action="theory-source" data-id="${esc(post.id)}">${icon('copy')}复制原帖链接</button></div></article>`;}).join('')}</div>`;
  }
  ...
  function theoryPage() {
    const all=window.JIBO_THEORY_POSTS||[], posts=filteredTheoryPosts();
    return `${pageHead('肌薄理论')}<figure class="theory-masthead"><img src="${esc(localImage('assets/theory/muscle-meme.png'))}" alt="漫画：你看我这肌肉，怎么敢打我的啊" width="902" height="510"></figure><section class="theory-library" aria-label="原帖内容库"><div class="section-head theory-library-head"><div><h2>原帖引用</h2></div><span class="tiny-note" id="theory-count" role="status" aria-live="polite">${posts.length} / ${all.length} 条原帖</span></div><div class="searchbar theory-searchbar">${icon('search')}<input id="theory-search" type="search" maxlength="120" value="${esc(theorySearch)}" placeholder="搜索作者或原文" aria-label="搜索原帖" autocomplete="off" enterkeyhint="search"></div><div class="theory-toolbar"><div class="filter-row theory-author-filters" role="group" aria-label="按原帖作者筛选">${[['all','全部作者'],['alan','邵艾伦'],['sun','孙宇晨']].map(([id,name])=>`<button class="filter ${theoryAuthor===id?'active':''}" data-action="theory-filter" data-author="${id}" aria-pressed="${theoryAuthor===id}">${name}</button>`).join('')}</div><div class="filter-row theory-topic-filters" role="group" aria-label="按原帖主题筛选">${theoryCategories.map(([id,name])=>`<button class="filter ${theoryCategory===id?'active':''}" data-action="theory-category" data-category="${id}" aria-pressed="${theoryCategory===id}">${name}</button>`).join('')}</div></div><div id="theory-results">${theoryResults()}</div></section><p class="theory-footnote">原帖节选，视频仅含封面。作者观点不等于训练建议。</p><button class="btn btn-secondary btn-wide" data-nav="plans">训练计划 ${icon('arrow')}</button>`;
  }
  ...
  function libraryPage() {
    ...
    return `${pageHead('动作库',`${EX.length} 个动作 · 内置图示与自己的照片`)}<div class="library-toolbar"><span class="small muted">支持自定义名称和照片。</span><button class="btn btn-primary" data-action="new-custom-exercise">${icon('plus')}自建动作</button></div><div class="searchbar">${icon('search')}<input id="exercise-search" type="search" value="${esc(search)}" placeholder="搜索动作、英文名称或器械……" aria-label="搜索动作"></div><div class="filter-row">${filters.map(f=>`<button class="filter ${group===f?'active':''}" data-action="filter" data-group="${f}" aria-pressed="${group===f}">${f}</button>`).join('')}</div><div id="exercise-grid">${exerciseGrid()}</div><div class="explainer">${icon('info')}<p>动作名称、照片和笔记保存在本机。</p></div><div class="section-head"><span class="tiny-note">当前草稿：${state.draft?state.draft.blocks.length:0} 个动作</span><button class="btn btn-primary" data-nav="home">返回训练 ${icon('arrow')}</button></div>`;
  }
  ...
  function refreshNutritionPreview() {
    ...
    syncFoodDraft();
    if([foodForm.quantity,foodForm.kcal100,foodForm.protein100].some(v=>v==='')){el.textContent='填写标签数值和份量。';return;}
    try {
  ...
  function progressPage() {
    ...
    return `<p class="plan-view-intro">已保存的训练记录。</p><div class="kpi-grid"><section class="panel"><div class="metric">${sessions.length}</div><p class="small muted">已归档训练</p></section><section class="panel"><div class="metric">${stats.sets}</div><p class="small muted">累计完成组数</p></section><section class="panel"><div class="metric">${new Set(state.sessions.filter(s=>C.sessionStats(s,EX).sets>0).map(s=>s.date)).size}</div><p class="small muted">累计训练天数</p></section></div><div class="dashboard"><div class="main-col"><section class="panel"><div class="panel-title"><h3>最近七天</h3><small>完成组数</small></div><div class="bar-chart" role="img" aria-label="最近七天完成组数：${bars.map(b=>b.date+' '+b.n+'组').join('，')}">${bars.map(b=>`<div class="bar-col"><b>${b.n}</b><div class="bar" style="height:${Math.max(3,b.n/max*95)}px;opacity:${b.n?'.9':'.18'}"></div><span>${b.date.slice(5).replace('-','/')}</span></div>`).join('')}</div></section><div class="section-head"><div><h2>训练档案</h2></div><button class="btn btn-primary btn-small" data-action="report">${icon('trophy')}训练小结</button></div><div class="stack">${sessions.length?sessions.map(s=>{const st=C.sessionStats(s,EX);return `<article class="history-card"><div class="row space"><h3>${s.date}</h3><button class="quiet-icon" data-action="history" data-id="${esc(s.id)}" aria-label="查看${s.date}训练">${icon('arrow')}</button></div><p>${s.blocks.map(b=>EX_MAP.get(b.exerciseId).name).map(esc).join(' · ')}</p><div class="history-tags"><span class="chip">${st.sets} 组</span><span class="chip">${st.reps} 次</span>${st.seconds?`<span class="chip">${st.seconds} 秒</span>`:''}<span class="chip">${fmt(st.volume)} kg·次</span></div></article>`;}).join(''):'<div class="empty"><div class="empty-icon">'+icon('chart')+'</div><h3>暂无训练记录</h3><p>完成训练并保存后可在这里查看。</p></div>'}</div></div><aside class="aside-col"><section class="panel"><div class="panel-title"><h3>体重记录</h3><small>选填</small></div>${weights.length?`<div class="weight-number">${fmt(weights[0].kg)} <small class="muted">kg</small></div><p class="tiny-note">最近记录 ${weights[0].date}</p>`:'<p class="small muted">暂无体重记录。</p>'}<form id="weight-form" style="margin-top:17px"><div class="form-grid"><label class="field">日期<input id="weight-date" type="date" required min="2000-01-01" max="2100-12-31" value="${today()}"></label><label class="field">体重 / kg<input id="weight-kg" type="number" inputmode="decimal" min="1" max="500" step="0.1" required placeholder="选填记录"></label></div><button class="btn btn-secondary btn-wide" type="submit" style="margin-top:14px">${icon('plus')}保存体重</button></form><div class="weight-log" style="margin-top:15px">${weights.map(w=>`<div class="list-item"><span class="small muted">${w.date}</span><div class="row" style="gap:4px"><strong class="small">${fmt(w.kg)} kg</strong><button class="quiet-icon" data-action="delete-weight" data-date="${w.date}" aria-label="删除${w.date}体重">${icon('x')}</button></div></div>`).join('')}</div><p class="footnote">同一天再次保存会更新该日数值。</p></section><section class="panel aside-card"><h3>计算说明</h3><p class="small muted" style="margin:12px 0">负重 × 次数、自重次数、计时秒数分别统计。</p><button class="btn btn-ghost btn-small" data-action="formulas">查看计算说明 ${icon('arrow')}</button></section></aside></div>`;
  }
  function history(id) {
  ...
  function receiveUpdate(raw,notify=true) {
    ...
    if(notify&&updateState.status==='available'&&hasAvailableUpdate()&&announcedUpdate!==String(updateState.latestCode)){
      announcedUpdate=String(updateState.latestCode);toast(`肌薄 ${updateState.latestVersion} 可更新，到设置查看。`);
    }
  }
  ...
  function updateStatusHTML() {
    if(!updateSupported())return `<p class="small muted">${isNative()?'当前安卓版本尚未提供应用内更新，请从发布页下载安装新版。':'安卓安装包请从 GitHub 发布页下载。'}</p><div class="update-actions"><a class="btn btn-primary" href="${RELEASES_URL}" target="_blank" rel="noopener noreferrer">打开 GitHub 发布页 ${icon('arrow')}</a><button class="btn btn-secondary" data-action="update-copy-link">${icon('copy')}复制下载页链接</button></div>`;
    const st=updateState,known=st.latestCode>st.currentCode&&!!st.latestVersion;
    const messages={idle:'检查 GitHub 上的新版本。',checking:'正在检查新版本……',available:'发现新版本，下载后即可安装。',downloading:st.progress>=100?'下载完成，正在校验安装包……':'正在下载安装包……',ready:'安装包已校验，可以开始安装。',installing:'请在安卓系统界面完成授权和安装确认。',upToDate:'当前已是最新版本。',error:'更新暂未完成，可以重试。'};
    let actions='';
  ...
  function refreshUpdateUI() {
    ...
    const progress=$('#update-progress');if(progress){progress.value=updateState.progress;$('#update-progress-label').textContent=updateState.progress+'%';$('#update-status-message').textContent=updateState.progress>=100?'下载完成，正在校验安装包……':'正在下载安装包……';}
  }
  function settingsPage() {
    return `${pageHead('设置与备份')}<div class="wide-content stack">${softwareUpdateCard()}<section class="panel"><form id="profile-form"><label class="field">昵称<input id="nickname" maxlength="30" value="${esc(state.profile.nickname)}" placeholder="训练者"></label><button class="btn btn-secondary btn-small" type="submit" style="margin-top:12px">保存昵称</button></form><div class="settings-line"><div><strong>首页引文</strong><p>显示邵艾伦的原帖短句。</p></div><button class="toggle ${state.profile.fun?'on':''}" role="switch" aria-checked="${state.profile.fun}" aria-label="首页引文" data-action="toggle-fun"></button></div><div style="margin-top:16px"><strong class="small">主题</strong><div class="theme-picker"><button class="theme-option ${state.profile.theme==='lean'?'active':''}" data-action="theme" data-theme="lean">绿色</button><button class="theme-option ${state.profile.theme==='blond'?'active':''}" data-action="theme" data-theme="blond">黄色</button></div></div></section><section class="panel"><div class="panel-title"><h3>本地数据与备份</h3>${icon('lock')}</div><p class="notice">${isNative()?'照片、计划和记录存在本机，自动云备份已关闭。':'记录存在当前浏览器；换浏览器或清理站点数据会影响记录。'}<br>卸载或清空前请导出备份。备份包含照片与记录，未加密。</p><div class="row wrap" style="margin-top:17px"><button class="btn btn-primary" data-action="export-json">${icon('download')}完整 JSON 备份</button><button class="btn btn-secondary" data-action="export-csv">训练 CSV</button><button class="btn btn-ghost" data-action="import-json">${icon('upload')}导入 JSON</button></div>${damagedRaw?'<button class="btn btn-danger btn-wide" style="margin-top:12px" data-action="export-raw">导出未解析的原始数据</button>':''}<input id="import-file" class="file-input" type="file" accept="application/json,.json" aria-label="选择 JSON 备份"><p class="footnote">导入会整体替换当前数据，请先备份。CSV 仅含训练记录，不能完整恢复。</p><div class="danger-zone"><button class="btn btn-danger btn-small" data-action="reset">清空所有本地记录</button><p class="footnote">包括动作照片、计划和训练记录。</p></div></section><section class="panel"><h3>关于肌薄</h3><div class="settings-line"><div><strong>肌薄 · v${APP_VERSION}</strong><p>本地训练记录工具，无账号。</p></div></div><div class="settings-line"><div><strong>素材与引用</strong><p>原帖、漫画与人物图标的权利归各权利人。本应用非官方出品。</p></div></div><button class="btn btn-ghost btn-small" style="margin-top:16px" data-action="formulas">${icon('info')}计算说明</button></section></div>`;
  }
  function formulas() {
    modal('<h2>计算说明</h2>',`<h3>1. 负重 × 次数</h3><div class="formula" style="margin:12px 0">20 kg × 12 次 = 240 kg·次\n20 kg × 12 次 = 240 kg·次\n20 kg × 10 次 = 200 kg·次\n总和 = 240 + 240 + 200 = 680 kg·次</div><p class="small muted">只计算勾选完成的有效组。空白不计，自重与计时分别统计。</p><div class="divider"></div><h3>2. 标签计算</h3><div class="formula" style="margin:12px 0">假设标签每100 mL：62 kcal、蛋白质3.4 g\n记录250 mL：份数 = 250 ÷ 100 = 2.5\n能量 = 2.5 × 62 = 155 kcal\n蛋白质 = 2.5 × 3.4 = 8.5 g\nkJ ÷ 4.184 = kcal</div><p class="small muted">按填写的标签计算，显示值保留小数。</p><div class="divider"></div><h3>3. 本周训练天数</h3><p class="small muted" style="margin-top:10px">从本周一到今天，保存过有效完成组的日期计为训练日。同一天只计一次，草稿不计入。</p>`);
  }
  ...
  function report(session = null) {
    ...
    lastReport=text;
    modal('<h2>训练小结</h2>',`<div class="meme-ticket"><h3>${stats.sets} 组，已记录。</h3><div class="report-text">${esc(text)}</div></div><p class="tiny-note">仅复制文字，不自动发布。</p><div class="modal-actions"><button class="btn btn-primary" data-action="copy-report">${icon('copy')}复制小结</button></div>`);
  }
  ...
  function timerPicker() {
    modal('<h2>休息计时</h2>',`<p class="small muted">退出应用后不发送计时通知。</p><div class="row wrap" style="margin-top:22px">${[30,60,90,120].map(s=>`<button class="btn btn-secondary" data-action="start-timer" data-seconds="${s}">${s} 秒</button>`).join('')}</div>`);
  }
  ...
  function handleAction(button) {
    ...
    else if(a==='theory-source'){const post=(window.JIBO_THEORY_POSTS||[]).find(p=>p.id===id);if(post)copyText(post.sourceUrl,'原帖链接已复制。');}
