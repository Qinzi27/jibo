...
下载 [肌薄 v1.5.2 Android 版](https://github.com/Qinzi27/jibo/releases/latest)，将 APK 传到手机后打开，按系统提示安装。可使用 [SHA-256 校验文件](dist/jibo-v1.5.2.apk.sha256) 核对安装包。
...
- “肌薄理论”：15 条公开可核实 X 原帖，按观点／训练／饮食／趣味分类，可搜索、按作者筛选、翻看截图及复制出处。长文采用原文短句与截图节选，视频只收录封面，不代表完整推文历史。
- 用户提供的漫画单格用作“肌薄理论”页头图，黄毛简笔画头像用作应用图标。
- 完整 JSON 备份与导入、训练 CSV 导出，另有食品标签计算工具。

## 文案与原帖

v1.5.2 删除自拟励志语、宣传标语、原帖摘要和旁白。首页显示邵艾伦的已核实短引文，点击署名可查看对应原帖截图；可在设置关闭首页引文。操作提示使用简短中文，理论页保留 15 条原文引用、作者、日期与出处。

漫画只作为“肌薄理论”页的一张静态头图。原帖中的 AI 生成配图保留原有事实标注。引用不表示原作者参与或为应用背书。
...
v1.5.2 通过 63 项核心测试、131 项原生策略检查、5 组打包安全测试及 327 项浏览器检查（主要界面 98、理论 64、手机交互 38、PWA 22、自建内容 42、模拟更新 38、安全 25）。另检查了 320／390／430 像素手机排版。APK 编译、对齐与签名核验通过，签名与旧版一致；Android 真机相机、文件选择器与覆盖安装尚待验证。
