...
下载 [肌薄 v1.3.0 Android 测试版](https://github.com/Qinzi27/jibo/releases/download/v1.3.0/jibo-v1.3.0.apk)（[仓库内安装包](dist/jibo-v1.3.0.apk)），传到手机后打开，按系统提示安装。可使用 [SHA-256 校验文件](dist/jibo-v1.3.0.apk.sha256) 核对安装包。
...
- “肌薄理论”：孙宇晨、邵艾伦的公开 X 原帖截图，按作者筛选、点击放大、复制出处，配有独立标注的趣味旁白。
...
v1.3.0 已通过 63 项核心测试与 238 项独立浏览器检查（主要界面 97、手机布局 38、自建动作和计划 42、理论与合并导航 40、PWA 离线 21）。APK 编译、对齐与签名验证通过。尚未连接 Android 真机或模拟器验证系统选图、拍照、原生持久化、软键盘及覆盖安装。
...
应用代码与原创素材采用 [MIT 许可](LICENSE)。X 原帖截图属于第三方素材，来源与截取范围见 [第三方素材说明](THIRD_PARTY_NOTICES.md)，不包含在 MIT 授权范围内。包名 `app.leancrew.local` 与既有存储标识保持不变，以保留升级兼容性。
