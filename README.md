...
下载 [肌薄 v1.2.1 Android 测试版](https://github.com/Qinzi27/jibo/releases/download/v1.2.1/jibo-v1.2.1.apk)（[仓库内安装包](dist/jibo-v1.2.1.apk)），传到手机后打开，按系统提示安装。可使用 [SHA-256 校验文件](dist/jibo-v1.2.1.apk.sha256) 核对安装包。
...
浏览器测试使用独立临时配置与随机本机端口，不使用日常浏览器记录。其他测试范围可通过 `--mobile-only`、`--pwa-only` 选择，省略范围参数运行主要界面回归。

v1.2.1 已通过 63 项核心测试、42 项图片与自建计划定向浏览器检查，APK 编译、对齐与签名验证通过。此前 v1.2 的完整浏览器基线为 181 项，本次未全部重跑。尚未连接 Android 真机或模拟器验证系统选图、拍照、原生持久化、软键盘及覆盖安装。
...
采用 [MIT 许可](LICENSE)。包名 `app.leancrew.local` 与既有存储标识保持不变，以保留升级兼容性。
